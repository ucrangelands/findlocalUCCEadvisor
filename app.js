(() => {
  ...
  function renderDetails(item){
    ...
    const advisorHtml=advisors.length?`<div class="advisor-list">${advisors.map((a,i)=>`<div class="advisor"><h3>${escapeHtml(a.name)}</h3><p>${advisors.length>1?`Advisor ${i+1}`:"UC Rangelands advisor"}</p>${a.url?`<a class="action-link" href="${escapeAttr(toHttps(a.url))}" target="_blank" rel="noopener noreferrer">View advisor profile <span aria-hidden="true">↗</span></a>`:""}</div>`).join("")}</div>`:'<div class="no-advisor"><strong>No advisor is listed for this county in the supplied workbook.</strong></div>';
    ...
    details.innerHTML=`<p class="eyebrow">County directory</p><h2 class="county-title">${escapeHtml(item.county)} County</h2><p class="county-subtitle">UC Rangelands contact information</p>${advisorHtml}${links.length?`<div class="link-list">${links.join("")}</div>`:""}`;
  }
